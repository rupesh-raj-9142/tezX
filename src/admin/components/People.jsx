import React, { useState } from 'react';

function People({ people = [], setPeople, companies = [], searchTerm: propSearchTerm, setSearchTerm: propSetSearchTerm }) {
  const [viewMode, setViewMode] = useState('list'); // 'list' or 'grid'
  const [localSearchTerm, setLocalSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  
  const searchTerm = propSearchTerm !== undefined ? propSearchTerm : localSearchTerm;
  const setSearchTerm = propSetSearchTerm !== undefined ? propSetSearchTerm : setLocalSearchTerm;
  
  // Modal States
  const [selectedPerson, setSelectedPerson] = useState(null);
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  
  // Form States
  const [form, setForm] = useState({
    name: '',
    email: '',
    phone: '',
    company: '',
    role: '',
    status: 'Active',
  });
  const [formErrors, setFormErrors] = useState({});

  // Filter & Search Logic
  const filteredPeople = people.filter(person => {
    const matchesSearch = 
      (person.name || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
      (person.email || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
      (person.role || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
      (person.company || '').toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesStatus = statusFilter === 'All' || person.status === statusFilter;
    
    return matchesSearch && matchesStatus;
  });

  const handleOpenAddModal = () => {
    setForm({
      name: '',
      email: '',
      phone: '',
      company: companies[0]?.name || '',
      role: '',
      status: 'Active',
    });
    setFormErrors({});
    setIsAddModalOpen(true);
  };

  const handleOpenEditModal = (person) => {
    setSelectedPerson(person);
    setForm({
      name: person.name,
      email: person.email,
      phone: person.phone || '',
      company: person.company || '',
      role: person.role || '',
      status: person.status || 'Active',
    });
    setFormErrors({});
  };

  const validateForm = () => {
    const errors = {};
    if (!form.name.trim()) errors.name = 'Name is required';
    if (!form.email.trim()) {
      errors.email = 'Email is required';
    } else if (!/\S+@\S+\.\S+/.test(form.email)) {
      errors.email = 'Invalid email address';
    }
    if (!form.role.trim()) errors.role = 'Role is required';
    setFormErrors(errors);
    return Object.keys(errors).length === 0;
  };

  const handleAddSubmit = (e) => {
    e.preventDefault();
    if (!validateForm()) return;

    const initials = form.name
      .split(' ')
      .map(n => n[0])
      .join('')
      .toUpperCase()
      .slice(0, 2);

    const newPerson = {
      id: `person-${Date.now()}`,
      name: form.name.trim(),
      email: form.email.trim(),
      phone: form.phone.trim(),
      company: form.company,
      role: form.role.trim(),
      status: form.status,
      avatar: initials || 'PP',
    };

    setPeople(prev => [newPerson, ...prev]);
    setIsAddModalOpen(false);
  };

  const handleEditSubmit = (e) => {
    e.preventDefault();
    if (!validateForm()) return;

    const updatedPeople = people.map(p => {
      if (p.id === selectedPerson.id) {
        const initials = form.name
          .split(' ')
          .map(n => n[0])
          .join('')
          .toUpperCase()
          .slice(0, 2);
          
        return {
          ...p,
          name: form.name.trim(),
          email: form.email.trim(),
          phone: form.phone.trim(),
          company: form.company,
          role: form.role.trim(),
          status: form.status,
          avatar: p.avatar && p.avatar.startsWith('http') ? p.avatar : (initials || 'PP')
        };
      }
      return p;
    });

    setPeople(updatedPeople);
    setSelectedPerson(null);
  };

  const handleDelete = (id, name) => {
    if (window.confirm(`Are you sure you want to delete ${name}?`)) {
      setPeople(prev => prev.filter(p => p.id !== id));
      if (selectedPerson?.id === id) {
        setSelectedPerson(null);
      }
    }
  };

  return (
    <div className="w-full flex flex-col gap-6 text-left">
      {/* Header section */}
      <header className="mb-4 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="font-headline-lg text-[32px] text-[#111111] font-bold tracking-tight mb-1">People & Contacts</h2>
          <p className="font-body-md text-[14px] text-[#8f8f95]">
            Manage team members, clients, and partners in your CRM network ({people.length} total).
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="bg-[#f3f3f5] rounded-xl p-1 flex gap-1 border border-[#ececec]">
            <button 
              onClick={() => setViewMode('list')}
              className={`px-3 py-1.5 rounded-lg text-[13px] font-bold transition-all ${
                viewMode === 'list' 
                  ? 'bg-white shadow-sm text-[#1769ff]' 
                  : 'text-[#8f8f95] hover:bg-black/5'
              }`}
            >
              Table View
            </button>
            <button 
              onClick={() => setViewMode('grid')}
              className={`px-3 py-1.5 rounded-lg text-[13px] font-bold transition-all ${
                viewMode === 'grid' 
                  ? 'bg-white shadow-sm text-[#1769ff]' 
                  : 'text-[#8f8f95] hover:bg-black/5'
              }`}
            >
              Card Grid
            </button>
          </div>
          <button 
            onClick={handleOpenAddModal}
            className="bg-[#1769ff] text-white px-4 py-2.5 rounded-xl text-[14px] flex items-center gap-1.5 font-bold hover:bg-[#0054e6] active:scale-95 transition-all shadow-md shadow-[#1769ff]/10 cursor-pointer"
          >
            <span className="material-symbols-outlined text-[18px]">person_add</span>
            Add Person
          </button>
        </div>
      </header>

      {/* Filters bar */}
      <div className="bg-white border border-[#ececec]/60 rounded-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
        {/* Search Input */}
        <div className="relative w-full md:flex-grow md:max-w-[480px] lg:max-w-[640px] transition-all duration-300">
          <span className="material-symbols-outlined absolute left-3 top-2.5 text-[20px] text-[#8f8f95]">search</span>
          <input
            type="text"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Search by name, company, email..."
            className="w-full h-10 pl-10 pr-4 bg-[#f7f7f8] border border-[#ececec] rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px]"
          />
          {searchTerm && (
            <button 
              onClick={() => setSearchTerm('')} 
              className="absolute right-3 top-2.5 text-[#8f8f95] hover:text-black"
            >
              <span className="material-symbols-outlined text-[18px]">close</span>
            </button>
          )}
        </div>

        {/* Filters Select */}
        <div className="flex items-center gap-3 w-full md:w-auto">
          <label className="text-[12px] font-bold text-[#8f8f95] uppercase whitespace-nowrap">Status Filter:</label>
          <div className="flex gap-2">
            {['All', 'Active', 'Inactive'].map((status) => (
              <button
                key={status}
                onClick={() => setStatusFilter(status)}
                className={`px-3 py-1.5 rounded-lg text-[12px] font-bold border transition-all ${
                  statusFilter === status 
                    ? 'bg-[#e2dfff] text-[#3323cc] border-[#e2dfff]' 
                    : 'bg-white border-[#ececec] text-[#444] hover:bg-[#f5f5f6]'
                }`}
              >
                {status}
              </button>
            ))}
          </div>
        </div>
      </div>

      {/* Main Content Area */}
      {filteredPeople.length === 0 ? (
        <div className="border border-dashed border-[#ececec] rounded-3xl py-16 px-8 text-center text-[#8f8f95] bg-[#f7f7f8]/50 animate-in fade-in duration-200">
          <span className="text-4xl mb-3 block">👥</span>
          <h4 className="text-[16px] font-extrabold text-[#111111] mb-1">No contacts found</h4>
          <p className="text-[13px] max-w-[320px] mx-auto">Try adjusting your filters or click "Add Person" to create a new record.</p>
        </div>
      ) : viewMode === 'list' ? (
        /* Table View */
        <div className="bg-white border border-[#ececec]/60 rounded-3xl shadow-sm overflow-hidden animate-in fade-in duration-200">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[#ececec] bg-[#f7f7f8]/40">
                  <th className="px-6 py-4 text-[12px] font-bold text-[#8f8f95] uppercase">Name</th>
                  <th className="px-6 py-4 text-[12px] font-bold text-[#8f8f95] uppercase">Role</th>
                  <th className="px-6 py-4 text-[12px] font-bold text-[#8f8f95] uppercase">Company</th>
                  <th className="px-6 py-4 text-[12px] font-bold text-[#8f8f95] uppercase">Contact Details</th>
                  <th className="px-6 py-4 text-[12px] font-bold text-[#8f8f95] uppercase">Status</th>
                  <th className="px-6 py-4 text-[12px] font-bold text-[#8f8f95] uppercase text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#ececec]/50">
                {filteredPeople.map((person) => (
                  <tr key={person.id} className="hover:bg-[#f7f7f8]/30 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        {person.avatar && person.avatar.startsWith('http') ? (
                          <img 
                            src={person.avatar} 
                            alt={person.name} 
                            className="w-10 h-10 rounded-full object-cover border border-[#ececec] shadow-sm"
                          />
                        ) : (
                          <div className="w-10 h-10 rounded-full bg-[#e2dfff] text-[#3323cc] text-[13px] flex items-center justify-center font-bold border border-[#e2dfff] shadow-sm">
                            {person.avatar || 'PP'}
                          </div>
                        )}
                        <div>
                          <div className="font-bold text-[#111111] group-hover:text-[#1769ff] transition-colors">{person.name}</div>
                          <div className="text-[12px] text-[#8f8f95]">{person.role}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-[14px] font-medium text-[#111111]">
                      {person.role}
                    </td>
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-1 rounded-lg text-[12px] font-bold bg-[#f3f3f5] text-[#111111] border border-[#ececec]">
                        {person.company}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-[13px] font-medium text-[#111111] flex items-center gap-1">
                          <span className="material-symbols-outlined text-[14px] text-[#8f8f95]">mail</span>
                          {person.email}
                        </span>
                        {person.phone && (
                          <span className="text-[11px] text-[#8f8f95] flex items-center gap-1 mt-0.5">
                            <span className="material-symbols-outlined text-[13px]">phone</span>
                            {person.phone}
                          </span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-[11px] font-extrabold ${
                        person.status === 'Active' 
                          ? 'bg-[#e2f9ee] text-[#107c41]' 
                          : 'bg-[#f3f3f5] text-[#8f8f95]'
                      }`}>
                        {person.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => handleOpenEditModal(person)}
                          className="p-1 text-[#8f8f95] hover:text-[#1769ff] rounded-full hover:bg-black/5 active:scale-90 transition-all"
                          title="Edit Person"
                        >
                          <span className="material-symbols-outlined text-[20px]">edit</span>
                        </button>
                        <button
                          onClick={() => handleDelete(person.id, person.name)}
                          className="p-1 text-[#8f8f95] hover:text-[#ba1a1a] rounded-full hover:bg-red-50 active:scale-90 transition-all"
                          title="Delete Person"
                        >
                          <span className="material-symbols-outlined text-[20px]">delete</span>
                        </button>
                      </div>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      ) : (
        /* Card Grid View */
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in duration-200">
          {filteredPeople.map((person) => (
            <div 
              key={person.id}
              className="bg-white border border-[#ececec]/60 rounded-3xl p-6 shadow-sm hover:shadow-md hover:border-[#1769ff]/30 transition-all flex flex-col justify-between group"
            >
              <div>
                <div className="flex justify-between items-start mb-4">
                  {person.avatar && person.avatar.startsWith('http') ? (
                    <img 
                      src={person.avatar} 
                      alt={person.name} 
                      className="w-14 h-14 rounded-full object-cover border-2 border-[#1769ff]/10 group-hover:scale-105 transition-transform duration-300 shadow-sm"
                    />
                  ) : (
                    <div className="w-14 h-14 rounded-full bg-[#e2dfff] text-[#3323cc] text-[18px] flex items-center justify-center font-black border-2 border-[#1769ff]/10 group-hover:scale-105 transition-transform duration-300 shadow-sm">
                      {person.avatar || 'PP'}
                    </div>
                  )}
                  <span className={`px-2.5 py-1 rounded-full text-[10px] font-extrabold tracking-wide uppercase ${
                    person.status === 'Active' 
                      ? 'bg-[#e2f9ee] text-[#107c41]' 
                      : 'bg-[#f3f3f5] text-[#8f8f95]'
                  }`}>
                    {person.status}
                  </span>
                </div>

                <h4 className="text-[18px] font-extrabold text-[#111111] group-hover:text-[#1769ff] transition-colors mb-1 truncate">{person.name}</h4>
                <p className="text-[13px] text-[#8f8f95] font-semibold mb-3">{person.role}</p>

                <div className="flex items-center gap-2 mb-4">
                  <span className="text-[11px] font-bold text-[#8f8f95] uppercase tracking-wider">Company:</span>
                  <span className="px-2 py-0.5 rounded bg-[#f3f3f5] text-[#111111] text-[12px] font-bold border border-[#ececec]">
                    {person.company}
                  </span>
                </div>

                <div className="space-y-2 border-t border-[#ececec]/50 pt-4 text-[13px]">
                  <div className="flex items-center gap-2 text-[#444] font-medium truncate">
                    <span className="material-symbols-outlined text-[16px] text-[#8f8f95]">mail</span>
                    {person.email}
                  </div>
                  {person.phone && (
                    <div className="flex items-center gap-2 text-[#444] font-medium">
                      <span className="material-symbols-outlined text-[16px] text-[#8f8f95]">phone</span>
                      {person.phone}
                    </div>
                  )}
                </div>
              </div>

              <div className="flex items-center justify-end gap-2 mt-6 pt-4 border-t border-[#ececec]/40">
                <button
                  onClick={() => handleOpenEditModal(person)}
                  className="px-3 py-1.5 bg-[#f3f3f5] hover:bg-[#e2dfff] hover:text-[#3323cc] text-[#444] font-bold text-[12px] rounded-lg transition-colors flex items-center gap-1 cursor-pointer"
                >
                  <span className="material-symbols-outlined text-[14px]">edit</span>
                  Edit
                </button>
                <button
                  onClick={() => handleDelete(person.id, person.name)}
                  className="px-3 py-1.5 hover:bg-red-50 text-[#8f8f95] hover:text-[#ba1a1a] font-bold text-[12px] rounded-lg transition-colors flex items-center gap-1 cursor-pointer"
                >
                  <span className="material-symbols-outlined text-[14px]">delete</span>
                  Delete
                </button>
              </div>
            </div>
          ))}
        </div>
      )}

      {/* ADD PERSON MODAL */}
      {isAddModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm transition-opacity" onClick={() => setIsAddModalOpen(false)}></div>
          <div className="bg-white border border-[#ececec] rounded-2xl w-full max-w-md shadow-2xl relative z-10 overflow-hidden transform transition-all animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-[#ececec] flex items-center justify-between">
              <div>
                <h3 className="text-[18px] font-bold text-[#111111]">Add New Contact</h3>
                <p className="text-[12px] text-[#8f8f95]">Create a new profile record in CRM</p>
              </div>
              <button onClick={() => setIsAddModalOpen(false)} className="text-[#8f8f95] hover:text-[#ba1a1a] transition-colors p-1 rounded-full hover:bg-[#f5f5f6]">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>

            <form onSubmit={handleAddSubmit} className="p-6 space-y-4">
              <div className="flex flex-col gap-1">
                <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Full Name *</label>
                <input
                  type="text"
                  required
                  value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  placeholder="e.g. Marcus Thorne"
                  className={`w-full h-10 px-3 bg-white border rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px] ${formErrors.name ? 'border-[#ba1a1a] ring-1 ring-[#ba1a1a]' : 'border-[#ececec]'}`}
                />
                {formErrors.name && <span className="text-[11px] text-[#ba1a1a] font-bold">{formErrors.name}</span>}
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Email *</label>
                  <input
                    type="email"
                    required
                    value={form.email}
                    onChange={(e) => setForm({ ...form, email: e.target.value })}
                    placeholder="e.g. name@company.com"
                    className={`w-full h-10 px-3 bg-white border rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px] ${formErrors.email ? 'border-[#ba1a1a] ring-1 ring-[#ba1a1a]' : 'border-[#ececec]'}`}
                  />
                  {formErrors.email && <span className="text-[11px] text-[#ba1a1a] font-bold">{formErrors.email}</span>}
                </div>

                <div className="flex flex-col gap-1">
                  <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Phone</label>
                  <input
                    type="text"
                    value={form.phone}
                    onChange={(e) => setForm({ ...form, phone: e.target.value })}
                    placeholder="+1 (555) 000-0000"
                    className="w-full h-10 px-3 bg-white border border-[#ececec] rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px]"
                  />
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Company</label>
                  {companies.length > 0 ? (
                    <select
                      value={form.company}
                      onChange={(e) => setForm({ ...form, company: e.target.value })}
                      className="w-full h-10 px-3 bg-white border border-[#ececec] rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px]"
                    >
                      {companies.map(c => (
                        <option key={c.id} value={c.name}>{c.name}</option>
                      ))}
                      <option value="Custom / Other">Custom / Other</option>
                    </select>
                  ) : (
                    <input
                      type="text"
                      value={form.company}
                      onChange={(e) => setForm({ ...form, company: e.target.value })}
                      placeholder="e.g. Acme Corp"
                      className="w-full h-10 px-3 bg-white border border-[#ececec] rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px]"
                    />
                  )}
                  {form.company === 'Custom / Other' && (
                    <input
                      type="text"
                      placeholder="Type company name..."
                      onChange={(e) => setForm({ ...form, company: e.target.value })}
                      className="w-full h-10 px-3 bg-white border border-[#ececec] rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px] mt-2"
                    />
                  )}
                </div>

                <div className="flex flex-col gap-1">
                  <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Designation / Role *</label>
                  <input
                    type="text"
                    required
                    value={form.role}
                    onChange={(e) => setForm({ ...form, role: e.target.value })}
                    placeholder="e.g. Lead Developer"
                    className={`w-full h-10 px-3 bg-white border rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px] ${formErrors.role ? 'border-[#ba1a1a] ring-1 ring-[#ba1a1a]' : 'border-[#ececec]'}`}
                  />
                  {formErrors.role && <span className="text-[11px] text-[#ba1a1a] font-bold">{formErrors.role}</span>}
                </div>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Status</label>
                <select
                  value={form.status}
                  onChange={(e) => setForm({ ...form, status: e.target.value })}
                  className="w-full h-10 px-3 bg-white border border-[#ececec] rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px]"
                >
                  <option value="Active">Active</option>
                  <option value="Inactive">Inactive</option>
                </select>
              </div>

              <div className="flex justify-end gap-2 pt-4 border-t border-[#ececec]">
                <button
                  type="button"
                  onClick={() => setIsAddModalOpen(false)}
                  className="px-4 py-2 text-[#8f8f95] hover:bg-[#f5f5f6] font-bold text-[13px] rounded-xl transition-all"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 bg-[#1769ff] hover:bg-[#0054e6] text-white font-bold text-[13px] rounded-xl transition-all shadow-md shadow-[#1769ff]/10"
                >
                  Create Person
                </button>
              </div>
            </form>
          </div>
        </div>
      )}

      {/* EDIT PERSON MODAL */}
      {selectedPerson && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm transition-opacity" onClick={() => setSelectedPerson(null)}></div>
          <div className="bg-white border border-[#ececec] rounded-2xl w-full max-w-md shadow-2xl relative z-10 overflow-hidden transform transition-all animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-[#ececec] flex items-center justify-between">
              <div>
                <h3 className="text-[18px] font-bold text-[#111111]">Edit Contact Profile</h3>
                <p className="text-[12px] text-[#8f8f95]">Modify details of the contact profile</p>
              </div>
              <button onClick={() => setSelectedPerson(null)} className="text-[#8f8f95] hover:text-[#ba1a1a] transition-colors p-1 rounded-full hover:bg-[#f5f5f6]">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>

            <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
              <div className="flex flex-col gap-1">
                <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Full Name *</label>
                <input
                  type="text"
                  required
                  value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  placeholder="e.g. Marcus Thorne"
                  className={`w-full h-10 px-3 bg-white border rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px] ${formErrors.name ? 'border-[#ba1a1a] ring-1 ring-[#ba1a1a]' : 'border-[#ececec]'}`}
                />
                {formErrors.name && <span className="text-[11px] text-[#ba1a1a] font-bold">{formErrors.name}</span>}
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Email *</label>
                  <input
                    type="email"
                    required
                    value={form.email}
                    onChange={(e) => setForm({ ...form, email: e.target.value })}
                    placeholder="e.g. name@company.com"
                    className={`w-full h-10 px-3 bg-white border rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px] ${formErrors.email ? 'border-[#ba1a1a] ring-1 ring-[#ba1a1a]' : 'border-[#ececec]'}`}
                  />
                  {formErrors.email && <span className="text-[11px] text-[#ba1a1a] font-bold">{formErrors.email}</span>}
                </div>

                <div className="flex flex-col gap-1">
                  <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Phone</label>
                  <input
                    type="text"
                    value={form.phone}
                    onChange={(e) => setForm({ ...form, phone: e.target.value })}
                    placeholder="+1 (555) 000-0000"
                    className="w-full h-10 px-3 bg-white border border-[#ececec] rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px]"
                  />
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Company</label>
                  {companies.length > 0 ? (
                    <select
                      value={form.company}
                      onChange={(e) => setForm({ ...form, company: e.target.value })}
                      className="w-full h-10 px-3 bg-white border border-[#ececec] rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px]"
                    >
                      {companies.map(c => (
                        <option key={c.id} value={c.name}>{c.name}</option>
                      ))}
                      <option value="Custom / Other">Custom / Other</option>
                    </select>
                  ) : (
                    <input
                      type="text"
                      value={form.company}
                      onChange={(e) => setForm({ ...form, company: e.target.value })}
                      placeholder="e.g. Acme Corp"
                      className="w-full h-10 px-3 bg-white border border-[#ececec] rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px]"
                    />
                  )}
                  {form.company === 'Custom / Other' && (
                    <input
                      type="text"
                      placeholder="Type company name..."
                      onChange={(e) => setForm({ ...form, company: e.target.value })}
                      className="w-full h-10 px-3 bg-white border border-[#ececec] rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px] mt-2"
                    />
                  )}
                </div>

                <div className="flex flex-col gap-1">
                  <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Designation / Role *</label>
                  <input
                    type="text"
                    required
                    value={form.role}
                    onChange={(e) => setForm({ ...form, role: e.target.value })}
                    placeholder="e.g. Lead Developer"
                    className={`w-full h-10 px-3 bg-white border rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px] ${formErrors.role ? 'border-[#ba1a1a] ring-1 ring-[#ba1a1a]' : 'border-[#ececec]'}`}
                  />
                  {formErrors.role && <span className="text-[11px] text-[#ba1a1a] font-bold">{formErrors.role}</span>}
                </div>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-[11px] font-bold text-[#8f8f95] uppercase">Status</label>
                <select
                  value={form.status}
                  onChange={(e) => setForm({ ...form, status: e.target.value })}
                  className="w-full h-10 px-3 bg-white border border-[#ececec] rounded-xl outline-none focus:ring-2 focus:ring-[#1769ff]/20 focus:border-[#1769ff] transition-all font-body-sm text-[13px]"
                >
                  <option value="Active">Active</option>
                  <option value="Inactive">Inactive</option>
                </select>
              </div>

              <div className="flex justify-end gap-2 pt-4 border-t border-[#ececec]">
                <button
                  type="button"
                  onClick={() => setSelectedPerson(null)}
                  className="px-4 py-2 text-[#8f8f95] hover:bg-[#f5f5f6] font-bold text-[13px] rounded-xl transition-all"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 bg-[#1769ff] hover:bg-[#0054e6] text-white font-bold text-[13px] rounded-xl transition-all shadow-md shadow-[#1769ff]/10"
                >
                  Save Changes
                </button>
              </div>
            </form>
          </div>
        </div>
      )}
    </div>
  );
}

export default People;
